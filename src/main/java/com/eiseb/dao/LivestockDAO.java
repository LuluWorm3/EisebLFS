...
public class LivestockDAO {
    ...
    public int insert(Livestock l) throws SQLException {
        String sql = "INSERT INTO livestock (tag, species, breed, category, gender, dob, current_value, status, image_path) " +
                     "VALUES (?,?,?,?,?,?,?,?,?)";
        try (Connection conn = DBConnection.getConnection(ctx);
            ...
            if (l.getDob() != null) ps.setDate(6, l.getDob());
            else                    ps.setNull(6, Types.DATE);
            ps.setBigDecimal(7, l.getCurrentValue() != null ? l.getCurrentValue() : BigDecimal.ZERO);
            ps.setString(8, l.getStatus() != null ? l.getStatus() : "Active");
            ps.setString(9, l.getImagePath());
            ps.executeUpdate();
    ...
    public void update(Livestock l) throws SQLException {
        String sql = "UPDATE livestock SET tag=?, species=?, breed=?, category=?, gender=?, dob=?, current_value=?, status=?, image_path=? WHERE id=?";
        try (Connection conn = DBConnection.getConnection(ctx);
            ...
            ps.setBigDecimal(7, l.getCurrentValue());
    ...
    public void updateValue(int id, BigDecimal value) throws SQLException {
        try (Connection conn = DBConnection.getConnection(ctx);
             PreparedStatement ps = conn.prepareStatement(
